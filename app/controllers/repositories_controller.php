	...
	function index($repo_url = null) {	
		...
		if($result[0]) {			
			...
			foreach($tags_name_count as $tag_info){
				...
				$cloud_data .= a href='.Routerurl(array('controller' = 'tags', 'action' = 'index'), true).tag_value=.$tag_info['Tag']['tag'].' style='.$tag_size.' color='0x.$this-color_aleatorio().' hicolor='0x.$this-color_aleatorio().'.$tag_info['Tag']['tag'].a;
			}
			$cloud_data .= tags;
	...
	function create() {
		...
		if(!empty($this-data)) {
			...
			if($this-Repository-validates()) {
				...
				CakeLogwrite('activity', Repository [name={$repository['Repository']['name']}] created);
				...
				if(Configureread('App.subdomains')) {
					$dom = Configureread('App.domain');
